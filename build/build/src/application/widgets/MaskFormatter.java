...
public class MaskFormatter {
	...
	private void maskTel8Dig() {
		...
		textField.setOnKeyTyped((KeyEvent evento) -> {
			...
			if (!"0123456789".contains(evento.getCharacter())) {
				/*
				 * usando o método consume,e como se o que o usuario digitou não tivesse efeito,
				 * ex ele digitou uma letra, mas essa letra não apareçe pq bloqueamos o evento
				 * que faz ela aperecer na tela.
				 */
				evento.consume();
			}
			/*
			 * verificamos se o caracter foi digitado ou apagado se caso apagado ele é igual
			 * a zero.
			 */
			if (evento.getCharacter().trim().length() == 0) {
				...
				case 9:
					/*
					 * subString so retornar os caracteres entre aquelas posições fazemos isso para
				...
				/*
				 * verificamos se já chegou o limite de numeros digitados lembrando que estamos
				 * contando todos os caracteres que estão visiveis
				 */
			...
			case 1:
				 ...
				 * caractere digitado e como se estivesse em um array então o primeiro caracter
				 * fica na posição 0
				 */
	...
	/**
	 * Exibi no componente o formato de máscara selecionado. Obs: Utilizar somente
	 * depois do método setMask()
	 */
